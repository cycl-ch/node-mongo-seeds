...
var config  = require(process.cwd() + "/seed");
 ...
 */
module.exports.seed = function(folderPath) {
  dir = folderPath || dir;
  var listOfFiles = null;

  loadFiles().then(function(list) {
    listOfFiles = list;
    return getConnection();
  }).then(function(db) {
    return seedDb(db, listOfFiles);
  }).then(function () {
    console.log('----------------------');
  ...
  return Q.promise(function(resolve, reject, notify) {
    ...
    fs.readdir(dir, function(err, files) {
      ...
      if(err || _.isUndefined(files)) {
        return reject(`Error reading '${dir}' folder`);
      } else {
